...
public class SPN implements ScheduleMethod {
    ...
    private void run(){
        ...
        while(!ProcessManager.getInstance().empty_readyQueue() && CoreManager.getInstance().selectable()){
            com.koreatech.ifteam.operating_system.model.Process getProcess = ProcessManager.getInstance().getMinBrustProcess(); // readyQ가 비어있을수도 있지. 이때 getProcess = null
            ...
            if(getCore.getProcess() != null){
                if(CoreManager.getInstance().operating(i)){ // 프로세스 처리 완료
                    ...
                    if(!ProcessManager.getInstance().empty_readyQueue()){ //레디 큐에 프로세스가 기다리고 있으면
                        com.koreatech.ifteam.operating_system.model.Process getProcess = ProcessManager.getInstance().getMinBrustProcess(); // 그 프로세스 가져와서
