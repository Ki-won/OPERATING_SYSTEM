...
public class HRRN implements ScheduleMethod {
    ...
    private void run(){
        ...
        while(!ProcessManager.getInstance().empty_readyQueue() && CoreManager.getInstance().selectable()){
            com.koreatech.ifteam.operating_system.model.Process getProcess = ProcessManager.getInstance().getMinRRProcess(SyncManager.getInstance().getClock()); // readyQ가 비어있을수도 있지. 이때 getProcess = null
